...
2. gusty (optionally) makes available to your custom operators a `task_id`, which is the file name. Just specify `task_id` as an argument in the operator's `__init__` method, and gusty will pass it in when building your task.

3. If you name your SQL tables after the `task_id`, you can detect table names, which in turn can be used as a list of dependencies.
...
We'll more or less take this example directly from the [gusty demo](https://github.com/chriscardillo/gusty-demo/blob/main/airflow/operators/jupyter_operator.py).
...
See the [Juypter Notebook Task Definition File example](https://github.com/chriscardillo/gusty-demo/blob/main/airflow/dags/stock_predictions/stock_data.ipynb) in the gusty demo, in the [stock_predictions DAG](https://github.com/chriscardillo/gusty-demo/tree/main/airflow/dags/stock_predictions). Notice how the first cell in the notebook is a YAML cell (see [Raw notebook](https://raw.githubusercontent.com/chriscardillo/gusty-demo/main/airflow/dags/stock_predictions/stock_data.ipynb)).
