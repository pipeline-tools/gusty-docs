...
2. **Decorative** orchestration, where tasks are functions and decorators are used to configure the tasks. Dependencies are often managed by passing the output of one function into the input of another. [Airflow's taskflow API](https://airflow.apache.org/docs/apache-airflow/stable/tutorial/taskflow.html#example-taskflow-api-pipeline) and [Dagster's entire API](https://docs.dagster.io/0.14.20/tutorial/ops-jobs/single-op-job#hello-op) are good examples of decorative orchestation.

3. **File-oriented** orchestration, where tasks are files, and dependencies are cleverly inferred or declared explicitly. Tools like [Mage](https://www.mage.ai/), [dbt](https://www.getdbt.com/), and [Orchest](https://www.orchest.io/) exemplify file-oriented orchestration.
