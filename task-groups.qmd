...
The above `METADATA.yml` will give the `spanish` task group a tooltip, when hoving over the node in Airflow's task group, and all tasks in the task group will be prefixed with `spanish_`, such as `spanish_oye` and `spanish_saludos`.
